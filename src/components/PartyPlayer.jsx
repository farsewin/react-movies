  ...
  (
    ...
    useEffect(() => {
      ...
      const syncStatusInterval = setInterval(() => {
        const nextSyncing = !!watchPartyRef.current?.isSyncing;
        setIsSyncing((prev) => (prev === nextSyncing ? prev : nextSyncing));
      }, 500);
    ...
    useEffect(() => {
      ...
      watchPartyRef.current.handlePartyCommand(remoteCmd);
    }, [roomState, isHost]);
    ...
    useEffect(() => {
      ...
      const handleMessage = (event) => {
        const isVidfast = /vidfast\.(pro|in|io|me|net|pm|xyz)/.test(
          event.origin,
        );
          ...
          if (playerEvent === "timeupdate") {
            ...
            if (now - (window.lastProgressUpdate || 0) > 5000) {
              ...
              updateWatchProgress(
                ...
                duration,
                isTV
                  ? { media_type: "tv", season, episode: uiEpisode }
                  : { media_type: "movie" },
              );
            }
          }
    ...
    return (
      ...
      >
          ...
          {shouldLoadIframe ? (
            <iframe
              ...
              className={`w-full h-full transition-transform duration-500 ease-out pointer-events-auto ${
                fsState === 2
                  ? "scale-[1.4] sm:scale-[1.15] object-cover"
                  : "scale-100"
              }`}
              allowFullScreen
        ...
        >
          {/* Top Bar */}
          <div
            className={`pt-2 px-6 py-4 flex items-center justify-between transition-all duration-500 transform pointer-events-auto bg-linear-to-b from-black/90 via-black/40 to-transparent ${showControls || (!isFullscreen && !isCinematic) ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"}`}
          >
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <span className="font-mono text-sm uppercase tracking-tighter hidden sm:block">
                  {roomCode}
                </span>
              </button>
            </div>
          ...
          <div className="relative pointer-events-none pb-2">
            {/* Sync Status */}
            <div
              className={`absolute bottom-20 left-6 z-[60] flex flex-wrap gap-2 transition-all duration-500 ${showControls || (!isFullscreen && !isCinematic) ? "opacity-100" : "opacity-0"}`}
            >
              {isHost ? (
                ...
                <div className="px-3 py-1 bg-indigo-600/90 backdrop-blur-sm rounded-lg text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 border border-indigo-400/20 shadow-lg">
                  <span
                    className={`size-1.5 bg-white rounded-full ${isSyncing ? "animate-ping" : "animate-pulse"}`}
                  />
            ...
            {/* Fullscreen Button */}
            <div
              className={`absolute bottom-20 right-6 z-[60] flex items-center gap-2 transition-all duration-500 pointer-events-auto ${showControls || (!isFullscreen && !isCinematic) ? "opacity-100" : "opacity-0 pointer-events-none"}`}
            >
              <button
                ...
                >
                  {fsState === 0 ? (
                    ...
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2.5}
                      d="M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5"
                    />
                  )}
                </svg>
                {isMobile
            ...
            {/* Chat Overlay */}
            <div
              className={`absolute bottom-28 right-6 z-[60] pointer-events-auto transition-all duration-300 ${isChatVisible ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
            >
              <ChatOverlay
        ...
        <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 z-40 pointer-events-none flex items-center justify-between px-[15%]">
          <div
            className={`text-white font-black text-xl flex flex-col items-center justify-center gap-2 transition-all duration-300 ${seekFeedback === "left" ? "scale-125 opacity-100" : "scale-75 opacity-0"}`}
          >
            <div className="size-16 bg-indigo-500/30 backdrop-blur-xl rounded-full flex items-center justify-center border border-indigo-400/50 shadow-[0_0_30px_rgba(99,102,241,0.5)]">
              <svg
                className="size-8 text-white opacity-90 drop-shadow-[0_2px_10px_rgba(255,255,255,0.3)] ml-[-0.25rem]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
          ...
          <div
            className={`text-white font-black text-xl flex flex-col items-center justify-center gap-2 transition-all duration-300 ${seekFeedback === "right" ? "scale-125 opacity-100" : "scale-75 opacity-0"}`}
          >
            <div className="size-16 bg-indigo-500/30 backdrop-blur-xl rounded-full flex items-center justify-center border border-indigo-400/50 shadow-[0_0_30px_rgba(99,102,241,0.5)]">
              <svg
                className="size-8 text-white opacity-90 drop-shadow-[0_2px_10px_rgba(255,255,255,0.3)] ml-[0.25rem]"
