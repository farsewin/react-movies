      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-dark-100/60 backdrop-blur-2xl border border-white/10 rounded-[32px] shadow-3xl flex flex-col md:flex-row p-8 md:p-10 gap-10 animate-in fade-in slide-in-from-bottom-8 duration-500 custom-scrollbar">
        ...
        {loading ? (
          <div className="w-full h-96 flex flex-col items-center justify-center gap-4">
            <Spinner />
            <p className="text-light-200 font-bold animate-pulse text-sm">Fetching detailed intel...</p>
          </div>
        ) : details ? (
          <>
            {/* Left: Poster */}
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              <div className="mb-6">
                <h3 className="text-white font-bold mb-2 uppercase text-xs tracking-widest opacity-50">Overview</h3>
              ...
              {details.genres && details.genres.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-white font-bold mb-2 uppercase text-xs tracking-widest opacity-50">Genres</h3>
                  <div className="flex flex-wrap gap-2">
                    {details.genres.map(g => (
                      <span key={g.id} className="bg-white/5 hover:bg-white/10 border border-white/5 px-3 py-1 rounded-lg text-xs font-bold text-light-100 transition-colors">
                        {g.name}
                      </span>
                    ))}
                  </div>
              ...
              {details.credits?.cast && details.credits.cast.length > 0 && (
                <div className="mb-8">
                  <h3 className="text-white font-bold mb-3 uppercase text-xs tracking-widest opacity-50">Top Cast</h3>
                  <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar">
                    {details.credits.cast.slice(0, 5).map(person => (
                      <div key={person.id} className="shrink-0 flex items-center gap-3 bg-white/5 p-2 rounded-xl border border-white/5">
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="text-white text-xs font-bold truncate max-w-[100px]">{person.name}</span>
                          <span className="text-light-200 text-[10px] opacity-70 truncate max-w-[100px]">{person.character}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="mt-auto pt-6 border-t border-white/5 flex gap-4">
                ...
                >
                  {isCreating ? <Spinner /> : (
                    <>
                      <svg className="size-5 transition-transform group-hover:scale-110" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="w-full py-20 text-center">
            <p className="text-red-400 font-bold">Failed to load content details.</p>
          </div>
        )}
      </div>
    </div>
