    ...
    const unsubscribe = client.subscribe(
      ...
      (response) => {
        ...
        if (
          ...
          setRooms((prev) =>
            prev.filter((r) => r.$id !== response.payload.$id),
          );
        }
      },
    );
      ...
      <div className="wrapper relative z-10">
        <nav className="flex justify-between items-center mb-16 px-2">
          <div className="flex items-center gap-8">
            <p
              onClick={() => navigate("/")}
              className="text-white font-black text-2xl tracking-tighter cursor-pointer hover:text-indigo-400 transition-colors"
            >
        ...
        <header className="mb-12 px-2">
          ...
          <p className="text-light-200 text-lg font-medium max-w-2xl">
            Watch movies and TV shows with friends in perfect sync. Join an
            existing room or create your own.
          </p>
        </header>
        ...
        <section className="bg-dark-100/30 backdrop-blur-xl border border-white/5 rounded-[40px] p-6 sm:p-10 shadow-3xl">
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              <Spinner />
              <p className="text-light-200 font-bold animate-pulse uppercase tracking-widest text-[10px]">
                Scanning for active parties...
              </p>
            </div>
          ) : rooms.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center">
              <div className="size-20 bg-indigo-600/10 rounded-full flex items-center justify-center mb-6 border border-indigo-500/20">
                <svg
                  className="size-10 text-indigo-400 opacity-50"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <p className="text-light-200 max-w-xs text-sm font-medium">
                Be the first to start a party! Go back home, find a movie, and
                invite your friends.
              </p>
              <button
                onClick={() => navigate("/")}
